...
#
# REBUILD feed files by scanning the articles directory from
# scratch. Parses article headers/metadata, sorts entries by
# publication date and then regenerates feed content to keep
# output consistent and deterministic.
#
# Usage:
#   rebuild-feeds.pl
#
...
sub rebuild_feed_file {
    ...
    my ($channel_link) =
      $xml =~ m{<link>(.*?)</link>}s;
    $channel_link =
      normalize_root_relative($channel_link);
    my ($channel_desc) =

...
# -------------------------
# Usage: convert-media.pl [--root DIR] [--apply|--check] [--remove-originals] [--no-color] [--verbose]
# Modes:
...
#   --remove-originals  Remove original files after successful conversion (apply mode only)
...
sub usage {
    print STDERR <<"USAGE";
Usage:
  $0 [--root DIR] [--apply|--check] [--remove-originals] [--no-color] [--verbose]
...
GetOptions(
    ...
    "remove-originals!" => \$remove_originals,
    "no-color!"         => \$no_color,
    "verbose!"          => \$verbose,
) or usage();

-d $root_dir or die "ERROR: root is not a directory: $root_dir\n";
if ($remove_originals && !$mode_apply) {
    die "ERROR: --remove-originals only makes sense with --apply\n";
}
...
sub run_capture {
    ...
    # Portable-ish capture without extra deps:
...
sub write_atomic {
    ...
    # Rename is atomic on POSIX within same filesystem (we create tmp in same dir).
...
logi("Found images=" . scalar(@images) . ", media=" . scalar(@media_candidates) . " under: $root_dir");

# -------------------------
# Tool selection (best available)
# -------------------------
my $magick = have_cmd("magick") ? "magick" : (have_cmd("convert") ? "convert" : "");
my $ffmpeg = have_cmd("ffmpeg") ? "ffmpeg" : "";
my $ffprobe = have_cmd("ffprobe") ? "ffprobe" : "";

# Only require tools if there are relevant files.
if (@images) {
    $magick or die_tool("Required tool not found: 'magick' (ImageMagick 7) or 'convert' (ImageMagick 6) for image conversion.");
}
if (@media_candidates) {
    $ffmpeg  or die_tool("Required tool not found: 'ffmpeg' for audio/video conversion.");
    $ffprobe or die_tool("Required tool not found: 'ffprobe' for audio/video classification.");
}
...
sub convert_image_to_png {
    ...
    # Keep it simple and deterministic-ish.
...
sub ffprobe_has_video {
    my ($path) = @_;
    my ($rc, $out, $err) = run_capture($ffprobe,
        "-v", "error",
        "-select_streams", "v:0",
...
sub convert_audio_to_vorbis_ogg {
    ...
    # Vorbis quality scale ~0..10; 5 is a reasonable default.
    my @cmd = (
        $ffmpeg, "-y",
        "-i", $src,
        "-vn",
...
sub video_is_canonical {
    ...
    # Also enforce Vorbis audio if audio exists
...
for my $dst (sort keys %target_to_sources) {
    ...
    if (@srcs > 1) {
        $collision = 1;
        loge("Target collision: multiple sources want to write the same output: $dst");
        for my $s (@srcs) {
            print STDERR "  - $s\n";
            mark_error($s);
        }
...
if ($collision) {
    ...
    # In apply mode, refuse to proceed with ambiguous outputs.
    die_tool("Refusing to convert due to output name collisions. Rename files to avoid collisions.");
}
...
if (!$mode_apply) {
    ...
    if (@need) {
        loge("Conversion is required (--check): " . scalar(@need) . " file(s) need conversion.");
        for my $i (0 .. $#need) {
            last if $i > 199;
            my $src = $need[$i];
            print STDERR "  - $src\n";
    ...
    if (@bad) {
        loge("Errors detected while classifying media: " . scalar(@bad) . " file(s).");
        for my $i (0 .. $#bad) {
            last if $i > 199;
            print STDERR "  - $bad[$i]\n";
        }

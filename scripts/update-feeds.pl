...
# questions for article info (slug, title, description, pubDate).
#-------------------------
...
my $slug_en =
  normalize_slug( question( 'English slug (without .html, e.g., gpl)', '' ) );
length $slug_en or loge('English slug is required.');

my $slug_es =
  normalize_slug( question( 'Spanish slug (without .html)', $slug_en . '-es' ) );

my $key = normalize_slug(
    question( 'Key for data/articles.json (used by redirect.js)', $slug_en ) );
validate_key($key);
...
my $pubdate =
  question( 'Publication date RFC2822 (e.g., Fri, 07 Mar 2025 10:00:00 GMT)',
    $today );
